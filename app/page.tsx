...
function Admin({onClose,t}:{onClose:()=>void;t:(a:string,b:string)=>string}) {
  ...
  const nav = [
    ...
    ["▶","videos",t("Video","影片管理")],
    ["₫","prices",t("Giá riêng","專屬價格")],
  ];
  const titles: Record<string,string> = {
    orders:t("Quản lý đơn hàng","訂單管理"),
    customers:t("Quản lý khách hàng","客戶管理"),
    products:t("Quản lý sản phẩm","產品管理"),
    videos:t("Thư viện video","影片資料庫"),
    prices:t("Bảng giá riêng","專屬價格表"),
  };
  return <div className="admin"><aside><div className="logo"><span>TĐ</span><b>TÂN ĐÔNG<small>ADMIN</small></b></div><p>{t("QUẢN LÝ HỆ THỐNG","系統管理")}</p>{nav.map(x=><button className={tab===x[1]?"active":""} onClick={()=>setTab(x[1])} key={x[1]}><i>{x[0]}</i>{x[2]}<small>{items[x[1] as keyof typeof items]}</small></button>)}<button className="back" onClick={onClose}>← {t("Về trang đại lý","返回經銷商頁")}</button></aside><section className="adminMain"><div className="adminHead"><div><p>TÂN ĐÔNG PRO</p><h1>{titles[tab]}</h1></div><button>＋ {t("Thêm mới","新增")}</button></div><div className="statGrid"><div><span>{t("Chờ xác nhận","等待確認")}</span><b>8</b><small>{t("↑ 3 hôm nay","↑ 今日 3 筆")}</small></div><div><span>{t("Đang xử lý","處理中")}</span><b>12</b><small>{t("Đúng tiến độ","進度正常")}</small></div><div><span>{t("Đã giao tháng này","本月已交貨")}</span><b>146</b><small>↑ 12,4%</small></div><div><span>{t("Doanh số tháng","本月訂單金額")}</span><b>842M</b><small>{t("VNĐ · dữ liệu mẫu","越南盾・示範資料")}</small></div></div><div className="adminTable"><div className="tableTools"><b>{tab==="prices"?t("Giá theo từng khách hàng","各客戶專屬價格"):t("Danh sách gần đây","最近資料")}</b><input aria-label={t("Tìm kiếm","搜尋")} placeholder={t("⌕ Tìm kiếm...","⌕ 搜尋…")}/></div><div className="tr header"><span>{t("MÃ / KHÁCH HÀNG","編號／客戶")}</span><span>{t("NGÀY","日期")}</span><span>{t("TRẠNG THÁI","狀態")}</span><span>{t("TỔNG TIỀN","總金額")}</span><span></span></div>{customers.concat(customers).slice(0,5).map((c,i)=><div className="tr" key={i}><span><b>{i%2?c.name:`#TD-260811-0${8-i}`}</b><small>{c.username} · {c.tier.toUpperCase()}</small></span><span>11/08/2026</span><span><i className="status">{t("Chờ xác nhận","等待確認")}</i></span><span><b>{[4860000,7325000,2190000,8920000,3450000][i].toLocaleString("vi-VN")} ₫</b></span><button aria-label={t("Thao tác khác","更多操作")}>•••</button></div>)}</div><div className="adminNote">ⓘ {t("Bản mẫu quản trị — mọi thay đổi chỉ là dữ liệu mô phỏng, chưa kết nối ERP.","管理後台原型——所有變更皆為模擬資料，尚未連接ERP。")}</div></section></div>
}
